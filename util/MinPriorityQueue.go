...
func Get_MinPriorityQueue_Instance() *MinPriorityQueue {
 ...
 * @param key 入队元素
 */
func (h *MinPriorityQueue) enQueue(key int) {
	//队列长度超出范围，扩容
...
func (h *MinPriorityQueue) deQueue() (int, error) {
	if h.size <= 0 {
 ...
 * 上浮调整
 */
func (h *MinPriorityQueue) upAdjust() {
	childIndex := h.size - 1
...
func (h *MinPriorityQueue) downAdjust() {
	// temp保存父节点值，用于最后的赋值
	parentIndex := 0
...
func (h *MinPriorityQueue) resize() {
	//队列容量翻倍
	newSize := h.size * 2
